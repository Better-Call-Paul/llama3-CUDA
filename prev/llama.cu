...
__global__ void wmma_gemm_kernel(int M, int N, int K, float alpha, const __half* A, const __half* B, float beta, float* C) {
    ...
    load_matrix_sync(D_frag, C_tile, N, mem_row_major);
...
class Transformer {
public:
    // Make config_ public to access in main
    Config config_;
...
// Implement the forward pass using custom softmax and WMMA GEMM
float* Transformer::forward(int token, int pos) {
    ...
    CUDA_CHECK(cudaMemcpy(x, w->token_embedding + token * dim, dim * sizeof(float), cudaMemcpyDeviceToDevice));

    // Forward all the layers
    for (int l = 0; l < p->n_layers; l++) {
        // 1. Attention RMSNorm
        rmsnorm_custom(s->xb, x, w->rms_att_weight + l * dim, dim);

        // 2. Compute q, k, v using WMMA GEMM
        // Convert x to __half
        __half* x_half;
        CUDA_CHECK(cudaMalloc(&x_half, dim * sizeof(__half)));
        convert_float_to_half(x, x_half, dim);

        // WMMA GEMM for wq * x
        wmma_gemm(dim, dim, dim, 1.0f, reinterpret_cast<const __half*>(w->wq + l * dim * dim), x_half, 0.0f, s->q);
        
        // WMMA GEMM for wk * x
        wmma_gemm(dim, kv_dim, dim, 1.0f, reinterpret_cast<const __half*>(w->wk + l * dim * dim), x_half, 0.0f, s->k);
        
        // WMMA GEMM for wv * x
        wmma_gemm(dim, kv_dim, dim, 1.0f, reinterpret_cast<const __half*>(w->wv + l * dim * dim), x_half, 0.0f, s->v);

        // Free the temporary x_half
        CUDA_CHECK(cudaFree(x_half));

        // 3. Apply RoPE (Relative Positional Encoding)
        // Placeholder: Implement RoPE rotation if necessary

        // 4. Compute attention scores (dot product of q and k)
        // For simplicity, assume attention scores are stored in s->att
        // Implement a custom attention score computation if needed

        // 5. Apply softmax using the custom softmax kernel
        softmax(p->n_heads, p->max_seq_len, reinterpret_cast<const __half*>(s->att), s->att);

        // 6. Compute weighted sum of values
        weighted_sum(s->xb, s->att, s->value_cache + l * p->max_seq_len * kv_dim, p->n_heads, p->max_seq_len, kv_dim, head_size);

        // 7. Attention output matmul
        // Convert xb to __half
        __half* xb_half;
        CUDA_CHECK(cudaMalloc(&xb_half, dim * sizeof(__half)));
        convert_float_to_half(s->xb, xb_half, dim);

        // WMMA GEMM for wo * xb
        wmma_gemm(dim, dim, dim, 1.0f, reinterpret_cast<const __half*>(w->wo + l * dim * dim), xb_half, 0.0f, s->xb2);

        // Free the temporary xb_half
        CUDA_CHECK(cudaFree(xb_half));

        // 8. Residual connection back into x
        add_elements(x, x, s->xb2, dim);

        // 9. FFN RMSNorm
        rmsnorm_custom(s->xb, x, w->rms_ffn_weight + l * dim, dim);

        // 10. FFN operations using WMMA GEMM
        // WMMA GEMM for w1 * xb
        __half* xb_half_ffn;
        CUDA_CHECK(cudaMalloc(&xb_half_ffn, dim * sizeof(__half)));
        convert_float_to_half(s->xb, xb_half_ffn, dim);
        wmma_gemm(dim, hidden_dim, dim, 1.0f, reinterpret_cast<const __half*>(w->w1 + l * dim * hidden_dim), xb_half_ffn, 0.0f, s->hb);
        CUDA_CHECK(cudaFree(xb_half_ffn));

        // WMMA GEMM for w3 * xb
        __half* xb_half_ffn_w3;
        CUDA_CHECK(cudaMalloc(&xb_half_ffn_w3, dim * sizeof(__half)));
        convert_float_to_half(s->xb, xb_half_ffn_w3, dim);
        wmma_gemm(dim, hidden_dim, dim, 1.0f, reinterpret_cast<const __half*>(w->w3 + l * dim * hidden_dim), xb_half_ffn_w3, 0.0f, s->hb2);
        CUDA_CHECK(cudaFree(xb_half_ffn_w3));

        // 11. Apply SiLU Activation and Element-Wise Multiplication
        silu_elementwise_mul(s->hb, s->hb2, hidden_dim);

        // 12. FFN output matmul
        wmma_gemm(hidden_dim, dim, dim, 1.0f, reinterpret_cast<const __half*>(w->w2 + l * dim * hidden_dim), s->hb, 0.0f, s->xb);

        // 13. Residual connection
        add_elements(x, x, s->xb, dim);
    }

    // Final RMSNorm
    rmsnorm_custom(x, x, w->rms_final_weight, dim);

    // Classifier matmul to get logits
    __half* x_half_final;
    CUDA_CHECK(cudaMalloc(&x_half_final, dim * sizeof(__half)));
    convert_float_to_half(x, x_half_final, dim);
    wmma_gemm(dim, p->vocab_size, dim, 1.0f, reinterpret_cast<const __half*>(w->wcls), x_half_final, 0.0f, s->logits_gpu);
    CUDA_CHECK(cudaFree(x_half_final));
    ...
    void generate(const std::string& prompt, int max_new_tokens){
        ...
        while(pos < max_new_tokens){
            ...
            if(pos < prompt_tokens.size() -1){
                ...
                for(int i =1; i < transformer.config_.vocab_size; i++){
                    if(logits[i] > max_p){
                        max_p = logits[i];
                        next = i;
                    }
